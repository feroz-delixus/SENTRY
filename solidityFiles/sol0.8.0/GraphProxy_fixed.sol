...
contract GraphProxy is GraphProxyStorage {
    ...
    function acceptUpgradeAndCall(bytes calldata data) external ifAdminOrPendingImpl {
        ...
        if (data.length > 0) {
            (bool success,) = _implementation().delegatecall(abi.encode(data));
    ...
    function _fallback() internal {
        ...
        assembly {
            ...
            impl := and(sload(IMPLEMENTATION_SLOT), 0xffffffffffffffffffffffffffffffffffffffff)
            if eq(impl,address()) {
                // (1) copy incoming call data
                calldatacopy(ptr, 0, calldatasize())
                mstore(0x40, add(ptr, calldatasize()))
                let outptr := add(ptr, calldatasize())
                // (2) forward call to logic contract
                success := delegatecall(gas(), impl, ptr, calldatasize(), outptr, 0)
        ...
        require(success,"can not forward the call transaction");
    }
